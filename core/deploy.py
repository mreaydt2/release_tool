from core import properties as prop, xml_file_reader as filereader, snowflake_manager as sfm
...
class Deploy(object):
    ...
    def __init__(self, target_database, cloning, properties):
        ...
        self.master_change_log_file = Path(self.change_log_directory, self.master_chnage_log_name)
        self.root_sql_directory = properties.get('root_sql_directory')
        self.properties = properties
        self.target_database = target_database
        self.cloning = cloning
        self.snowflake_manager = Deploy.get_snowflake_manager(target_database=target_database,
                                                              properties=properties)

    @staticmethod
    def get_snowflake_manager(target_database, **properties):
        sf = sfm.SnowflakeManager(conn=sfm.get_conn(properties=properties),
                                  target_database=target_database)
        return sf

    def _deployable_changes(self):
        for change_history in self.snowflake_manager.get_database_chnage_history():
